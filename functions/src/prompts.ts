/**
 * Server-side copy of the recipe prompts.
 *
 * These live here rather than being sent up from the browser on purpose: if the
 * client supplied the system instruction, anyone could point the proxy at
 * arbitrary generation and spend the project's Gemini quota on whatever they
 * liked, under our organisation's account. The client only ever sends the user's
 * own messages and (for a variation) which recipe it is varying.
 *
 * Kept in sync with src/lib/prompts.ts, which the browser no longer uses for
 * generation. If you edit one, edit both.
 */
