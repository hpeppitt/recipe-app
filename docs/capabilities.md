...
| Verified-email gate on contributing | shipped | verified email | `firestore.rules` (`verified()`), `components/auth/EmailLinkingForm.tsx` | Deployed 2026-08-10, replacing the invite/membership design in roadmap 1.6. Publishing, suggesting and replying need `email_verified`; reading, favouriting and following stay open to anonymous accounts. Emulator-verified: 8/8 rule assertions, and the anonymous-upgrade path yields the claim on a freshly minted token with no forced refresh. |
| Sign-up call to action | shipped | firebase | `components/auth/EmailLinkingForm.tsx` (`tone="invite"`) | The block is an offer, not an error. Anonymous and signed-out users hitting `/create` or `/recipe/:id/vary` get a full-page sign-up screen instead of the composer; the suggest dialog swaps its form for the same call to action. One form, two tones: `warning` for the profile banner, `invite` for the gates. Picks `linkEmail` for an anonymous account (preserving the uid) and `sendEmailLink` for a signed-out visitor. |
| Generation gated to verified users | shipped | verified email | `pages/RecipeChatPage.tsx` | The one gate rules cannot enforce: Gemini is called client-side through Firebase AI Logic, so Firestore rules never see it. Blocks in front of the composer rather than at save time, so nobody spends a generation before being told. |
...
| Client error reporting | shipped | firebase | `services/telemetry.ts`, `lib/telemetry.ts`, `clientErrors` collection | Global `window.onerror` and `unhandledrejection` hooks plus 12 instrumented call sites that previously only reached the console. Write-only Firestore collection: no client can read it back. Capped at 10 reports per session, deduplicated by source+route+message, with a circuit breaker after 3 failed sends. Route only, never the full URL, since `/shared`'s hash carries a whole recipe and the sign-in query carries a live `oobCode`. Rules deployed and verified in production 2026-08-08. |
