...
| Client error reporting | partial | firebase | `services/telemetry.ts`, `lib/telemetry.ts`, `clientErrors` collection | Global `window.onerror` and `unhandledrejection` hooks plus 12 instrumented call sites that previously only reached the console. Write-only Firestore collection: no client can read it back. Capped at 10 reports per session, deduplicated by source+route+message, with a circuit breaker after 3 failed sends. Route only, never the full URL, since `/shared`'s hash carries a whole recipe and the sign-in query carries a live `oobCode`. **Partial until the rules deploy lands** — until then writes are denied and the breaker opens, silently and harmlessly. |
